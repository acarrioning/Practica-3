 ...
 //gameBoard;
	...
	//gameBoard = new GameBoard();
        ...
        it("GameBoard.overlap", function(){
               var referenceObject = {x: 0, y: 0, w: 10, h:10};
               var collideObject = {x: 4, y: 5, w: 10, h:10};
               var noCollideObject = {x: 15, y: 15, w: 10, h:10};
            
               //Compruebo el funcionamiento de overlap               
               expect(board.overlap(referenceObject, collideObject)).toEqual(true);
               expect(board.overlap(referenceObject, noCollideObject)).toEqual(false);
        ...
        it("GameBoard.collide", function(){
               var referenceObject = {x: 0, y: 0, w: 10, h:10, type:0};
               var collideObject = {x: 4, y: 5, w: 10, h:10, type:2};
               var noCollideObject = {x: 15, y: 15, w: 10, h:10, type:4};
               ...
               expect(board.collide(referenceObject, collideObject.type)).toBe(collideObject);
               expect(board.collide(referenceObject, noCollideObject.type)).toBe(false);
...
/*
   it("GameBoard.step()", function(){
        var obj1={"test":"sa"};
        expect(gameBoard.step(obj1)).toBe("")
    });


  it("GameBoard.detect()", function(){
       gameBoard.objects=[{obj1:"hockey"},{obj1:"Tennis"}];
       var test= function(){
                return this.obj1== "hockey";
        };
       expect(gameBoard.detect(test).toBe(gameBoard.objects[0])); 
